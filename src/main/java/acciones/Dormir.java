...
public class Dormir {
    ...
    public void hacerDormir(LocalTime tiempoADormir, Mascota mascota){
        if (mascota.isLive()){
            ...
            System.out.println("Lo siento esta mascota no ser encuentra operatica desde " + mascota.getFechaMuerte());
        }
    }

    private void modificarEstado(int horasDormidas, int minutosDormidos, Mascota mascota){
        //por cada 10 minutos dormidos,  el nivel de energia aumentara 5 puntos
        int munutosTotales = (horasDormidas * 60) + minutosDormidos;
        mascota.setNivelEnergia(mascota.getNivelEnergia() + ((munutosTotales*5)/10));
    }
}
